...
ANALYSIS_SYSTEM_PROMPT = """你是一个专业的代码分析工程师。你的任务是从项目源代码中识别功能模块，并将相关的前端页面路由和 REST API 端点按功能分组归类。

## 分析要求

### 第一步：识别功能模块（Feature Group）
在提取页面和 API 之前，先从整体上理解项目的业务功能结构：
- 按业务功能域或技术模块划分功能点（如"用户管理"、"订单处理"、"Vue TodoMVC"）
- **不要**按路由文件或代码文件划分
- 每个功能点应包含 2-10 个相关页面/API，避免单条目功能点
- 也不要将所有内容归入一个超大功能点
- 功能点名称应简洁明了（2-6个字），如"用户管理"、"商品管理"、"系统设置"

### 第二步：提取页面和 API 并归入功能点
...
### 返回格式
返回一个 JSON 对象（不要用 markdown 代码块包裹），按功能模块分组，结构如下：
{
  "features": [
    {
      "name": "用户管理",
      "description": "用户的增删改查和权限管理",
      "pages": [
        {
          "path": "/users",
          "name": "用户管理页",
      ...
      "apis": [
        {
          ...
          "name": "获取系统设置",
          "description": "读取系统配置项",
          "source_file": "src/controllers/SettingsController.java",
          "params": [],
          "response_fields": []
        }
      ]
...
### 重要约束
- 每个功能点应包含 2-10 个页面/API（pages + apis 总数），太少或太多的分组都不合理
- 如果项目只有少量页面/API（不足3个），可以归入一个功能点
...
- 每个功能点的 name 必须有值，description 可以简短
- 只返回 JSON，不要其他文字"""
...
def _analyze_with_cli(project, local_path: str) -> str:
    ...
    user_prompt = f"""请分析当前目录下的项目代码，识别功能模块，并提取所有前端页面路由和 REST API 端点，按功能分组归类。
...
请分析以上项目代码，识别功能模块，并提取所有前端页面路由和 REST API 端点，按功能分组归类。"""
...
def _parse_analysis_response(raw_text: str) -> list:
    ...
    支持两种输入格式（向后兼容）：
    1. 新格式: {"features": [{name, description, pages: [...], apis: [...]}]}
       → 每个 item 增加 feature_group 字段（来自所属 feature 的 name）
    2. 旧格式: {"pages": [...], "apis": [...]}
       → feature_group 为空字符串

    输出格式: [
      {"type": "page"|"api", "path", "name", "method", "description", "source_file",
       "elements": [...], "params": [...], "response_fields": [...], "feature_group": ""},
      ...
    ]

    新字段(elements/params/response_fields/feature_group)缺失时使用默认值，保持向后兼容。
    """
    # 尝试提取 JSON
    try:
