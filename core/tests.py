...
class BatchGenerateViewTest(TestCase):
    ...
    def test_batch_generate_success(self, mock_generate):
        """batch-generate 现在异步执行，立即返回 generating 状态"""
        mock_generate.return_value = [
        ...
        # 异步模式立即返回 generating 状态
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data['status'], 'generating')
    ...
    def test_batch_generate_ai_failure(self, mock_generate):
        """batch-generate 异步模式：AI 错误不阻塞 HTTP 响应，返回 generating"""
        ...
        # 异步模式立即返回 generating，错误在后台线程中处理
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data['status'], 'generating')
...
class ExecutionRecordListSerializerTest(TestCase):
...
class BatchConvertScriptsTest(TestCase):
    """测试批量脚本转换 API"""
    ...
    def setUp(self):
        ...
        self.tc_login = TC_Model.objects.create(
            project=self.project, name='登录测试', steps='步骤', expected_result='结果',
            feature_group='用户登录',
        )
        self.tc_order = TC_Model.objects.create(
    ...
    def test_batch_convert_empty_feature_group(self):
        """空功能点（未分组）应正确筛选"""
        tc_no_group = TC_Model.objects.create(
