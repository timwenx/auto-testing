...
### 第 0 阶段：利用已提供的上下文（如果有）
- 如果上方「页面元素信息」或「API 参数信息」section 非空，说明已通过代码分析提取了关键信息
- **页面测试**: 直接使用提供的选择器操作元素，跳过代码探索阶段
- **API 测试**: 使用提供的参数构造请求，跳过代码探索阶段
- 如果没有提供上下文信息，按照下面的标准流程执行

### 第 1 阶段：探索代码（仅在无上下文时需要）
